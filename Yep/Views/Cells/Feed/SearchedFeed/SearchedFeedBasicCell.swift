...
class SearchedFeedBasicCell: UITableViewCell {
    ...
    lazy var skillButton: UIButton = {
        let button = UIButton()
        let tintColor = UIColor(red: 199/255.0, green: 199/255.0, blue: 204/255.0, alpha: 1)
    ...
    func configureWithFeed(feed: DiscoveredFeed, layout: SearchedFeedCellLayout, keyword: String?) {
        ...
        if let keyword = keyword {
            messageTextView.attributedText = text.yep_hightlightSearchKeyword(keyword, baseFont: UIFont.feedMessageFont(), baseColor: UIColor.yepMessageColor())
