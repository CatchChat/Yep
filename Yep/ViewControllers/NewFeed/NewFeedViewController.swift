...
//                self?.imageAssets = imageAssets
    ...
    @objc private func post(sender: UIBarButtonItem) {
        ...
        case .Default:
            ...
            mediaImages.forEach({ image in
                ...
                if let image = image.resizeToSize(fixedSize, withInterpolationQuality: CGInterpolationQuality.High), imageData = UIImageJPEGRepresentation(image, 0.95) {
                    ...
                    s3UploadFileOfKind(.Feed, withFileExtension: .Image, inFilePath: nil, orFileData: imageData, mimeType: MessageMediaType.Image.mineType, failureHandler: { (reason, errorMessage) in
        ...
        case .Voice(let feedVoice):
            ...
            s3UploadFileOfKind(.Feed, withFileExtension: .Audio, inFilePath: feedVoice.fileURL.path, orFileData: nil, mimeType: MessageMediaType.Audio.mineType, failureHandler: { (reason, errorMessage) in
